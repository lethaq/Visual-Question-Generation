"""This script is used to question generation.
"""
...
#os.environ['CUDA_VISIBLE_DEVICES'] = '2,3'  # for several GPUs
...
def create_model(args, vocab, embedding=None):
    ...
    vqg = VQGRaD(len(vocab), args.max_length, args.hidden_size,
                 args.num_categories,
...
def evaluate(vqg, data_loader, criterion, l2_criterion, args):
    ...
    for iterations, (images, questions, answers,captions,
        ...
        caption_features = vqg.encode_answers(captions, alengths)
        category_features = vqg.encode_categories(categories)
        mus, logvars = vqg.encode_into_z(image_features, answer_features,caption_features,category_features)
        ...
        #kl_loss =vae_loss(outputs, targets, mus, logvars, criterion)
        ...
        if not args.no_image_recon or not args.no_answer_recon:
            ...
            caption_targets = caption_features.detach()
            category_targets = category_features.detach()
            recon_image_features, recon_answer_features,recon_caption_features,recon_category_features = vqg.reconstruct_inputs(
                    image_targets, answer_targets,caption_targets,category_targets)
            ...
            if not args.no_answer_recon:
                ...
                recon_cap_loss = l2_criterion(recon_caption_features, caption_targets)
                recon_c_loss = l2_criterion(recon_category_features, category_targets)
                total_recon_answer_loss += recon_a_loss.item()
                total_recon_caption_loss += recon_cap_loss.item()
        ...
        if iterations % args.log_step == 0:
             ...
             logging.info('Time: %.4f, Step [%d/%d], gen loss: %.4f, '
                          'KL: %.4f, I-recon: %.4f, A-recon: %.4f, Cap-recon  %.4f, C-recon: %.4f,'
                          'z-t-KL: %.4f, t-KL: %.4f'
                         % (delta_time, iterations, total_steps,
                            total_gen_loss/(iterations+1),
                            total_kl/(iterations+1),
                            total_recon_image_loss/(iterations+1),
                            total_recon_answer_loss/(iterations+1),total_recon_caption_loss/(iterations+1),total_recon_category_loss/(iterations+1),
                            total_z_t_kl/(iterations+1),
                            total_t_kl_loss/(iterations+1)))
    total_info_loss = total_recon_image_loss + total_recon_answer_loss+ total_recon_caption_loss+total_recon_category_loss
    #total_info_loss+=total_info_loss+kl_loss
...
def compare_outputs(images, questions, answers,captions, categories,
    ...
    outputs = vqg.predict_from_answer(images, answers,captions,categories, lengths=alengths)
...
def train(args):
    ...
    for epoch in range(args.num_epochs):
        for i, (images, questions, answers, captions,
                categories, qindices) in enumerate(data_loader):
            n_steps += 1
            ...
            if (args.eval_every_n_steps is not None and
                ...
                compare_outputs(images, questions, answers,captions, categories,
            ...
            if not args.no_answer_recon or not args.no_image_recon:
                ...
                if not args.no_answer_recon:
                    ...
                    total_info_loss += args.lambda_a * recon_c_loss
                    total_info_loss += args.lambda_a * recon_cap_loss
                    recon_answer_loss = recon_a_loss.item()
                    recon_caption_loss= recon_cap_loss.item()
            ...
            if i % args.log_step == 0:
                ...
                logging.info('Time: %.4f, Epoch [%d/%d], Step [%d/%d], '
                             'LR: %f, gen: %.2f, KL: %.2f, '
                             'I-recon: %.2f, A-recon: %.2f, Cap-recon: %.2f,  C-recon: %.2f, z-t-KL: %.2f, '
                             't-KL: %.2f, kl_loss: %.2f'
                             % (delta_time, epoch, args.num_epochs, i,
                                total_steps, gen_optimizer.param_groups[0]['lr'],
                                gen_loss, kl_loss,
                                recon_image_loss, recon_answer_loss,recon_caption_loss,recon_category_loss,
                                z_t_kl, t_kl,kl_loss))
...
if __name__ == '__main__':
    ...
    # Session parameters.
    parser.add_argument('--model-path', type=str, default=dir + 'weights/tf2/',
    ...
    # Data parameters.
    parser.add_argument('--vocab-path', type=str, default=dir + 'Dataset/vocab_vqgrad.json',
                        help='Path for vocabulary wrapper.')
    parser.add_argument('--dataset', type=str, default=dir + 'Dataset/vqgrad_train_dataset.hdf5',
                        help='Path for train annotation json file.')
    parser.add_argument('--val-dataset', type=str, default=dir + 'Dataset/vqgrad_test_dataset.hdf5',
                        help='Path for train annotation json file.')
    parser.add_argument('--train-dataset-weights', type=str, default=dir + 'Dataset/VQGRaD_train_dataset_weights.json',
                        help='Location of sampling weights for training set.')
    parser.add_argument('--val-dataset-weights', type=str, default=dir + 'Dataset/VQGRaD_test_dataset_weights.json',
                        help='Location of sampling weights for training set.')
    parser.add_argument('--cat2name', type=str, default=dir + 'Dataset/cat2name.json',
    ...
    parser.add_argument('--num-att-layers', type=int, default=20,
