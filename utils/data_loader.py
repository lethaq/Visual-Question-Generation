...
class VQGRaDDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        clength = caption.size(0) - caption.eq(0).sum(0).squeeze()
        qlength = question.size(0) - question.eq(0).sum(0).squeeze()
        if self.transform is not None:
            image = self.transform(image)
        return (image, question, answer,caption, answer_type,
                qlength.item(), alength.item(), clength.item())
...
def collate_fn(data):
    ...
    images, questions, answers,captions, answer_types, qlengths, _,_ = zip(*data)
    ...
    return images, questions, answers,captions, answer_types, qindices
...
def get_loader(dataset, transform, batch_size, sampler=None,
    ...
    """
    vqgrad = VQGRaDDataset(dataset, transform=transform, max_examples=max_examples,
                       indices=indices)
    data_loader = torch.utils.data.DataLoader(dataset=vqgrad,
